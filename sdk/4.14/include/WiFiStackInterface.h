...
public:
    ...
    virtual int scan_passive(const char *ssid, const unsigned char channel);
    /** scan a network
     *  @param ssid     Access Point to scan. could be NULL
     *  @param channel  Channel scan. could be 0
     *  @active, 0:passive scan; 1:active scan.
     *  @scan_time, scan time, unit is senond
     *  @country_code,default 0:JP(1~14), 1:NA(1~11), 2:EU(1~13)
     *  @return         number of scan result, negative on failure
    ...
    */
    ...
    virtual int scan_result(rda59xx_scan_result *bss_list, unsigned int num);
     ...
     */
    virtual int ap_join_info(rda59xx_apsta_info_t *sta_list, unsigned int num);
    ...
    /** Gets the current current for active apmode
     *  @return         Current channel of the AP mode
     */
    virtual int8_t get_channel_ap();
