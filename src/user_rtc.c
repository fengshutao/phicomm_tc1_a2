...
USER_FUNC void rtc_thread_func(void * arg){
	...
	int i, j;
	char task_flag[PLUG_NUM] = { -1, -1, -1 ,-1,-1};   //��¼Ҫ�ĸ�����Ҫ��������
 ...
 void USER_FUNC rtctime_timeout_handler( void* arg ){
  ...
  rtctimes=rtctimes+1;
  t=rtctimes+8*3600;  
  user_time_t=gmtime(&t); //ʱ��ṹ��
}


 void USER_FUNC rtctime_init(void ){
	if((user_rtc_timer = hftimer_create("TIMER2",1000,1,TIMER2_ID,rtctime_timeout_handler,0))==NULL)//ÿһ���һ,ģ��ʱ������
  {
