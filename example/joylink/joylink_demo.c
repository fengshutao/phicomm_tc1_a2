...
#endif
uint8_t dhcp_ok =0;
static int hf_atcmd_nobind(pat_session_t s,int argc,char *argv[],char *rsp,int len)
{
	if(argc != 0)
		return -4;
    extern void clr_jdArgs();
	clr_jdArgs();
	return 0;
}
#define APP_VER "jd_v1.0_20181206"

static int hf_cmd_get_version(pat_session_t s,int argc,char *argv[],char *rsp,int len)
{
	if(argc==0)
	{ 
		sprintf(rsp,"=%s(%s %s)",APP_VER,__DATE__,__TIME__);
...
static int sys_event_callback( uint32_t event_id,void * param)
{
	switch(event_id)
	{
		...
		case HFE_WIFI_STA_DISCONNECTED:
			hfservice_cloud_data_recv(HFSERV_EVENT_WIFI_DISCONNECT, NULL, 0);
		...
		case HFE_DHCP_OK:
			{
				...
				p_ip = (uint32_t*)param;
				dhcp_ok =1;
				hfservice_cloud_data_recv(HFSERV_EVENT_WIFI_CONNECT, NULL, 0);
				u_printf("dhcp ok %s!\n", inet_ntoa(*p_ip));
			}
			break;
		case HFE_SMTLK_OK:
...
static int USER_FUNC uart_recv_callback(uint32_t event,char *data,uint32_t len,uint32_t buf_len)
{
	...
	if(hfsys_get_run_mode() == HFSYS_STATE_RUN_CMD)
		return len;
	hfservice_uart_data_recv((unsigned char *)data, len);
	return 0;
}
