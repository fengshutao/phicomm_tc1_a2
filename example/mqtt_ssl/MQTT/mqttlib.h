...
#include <stdint.h>
	...
	#define MQTT_CONF_CLIENTID_LENGTH 64
#endif
	...
	#define MQTT_CONF_PASSWORD_LENGTH 32 // Recommended by MQTT Specification (12 + '\0')
#endif
...
typedef struct {
	...
	// Connection info
	char clientid[MQTT_CONF_CLIENTID_LENGTH+1];
	// Auth fields
	char username[MQTT_CONF_USERNAME_LENGTH+1];
	char password[MQTT_CONF_PASSWORD_LENGTH+1];
	// Will topic
	uint8_t will_retain;
	uint8_t will_qos;
	uint8_t will_flag;
...
/** mqtt_puback.
 *  After receiving the push message, reply to push ack
 *  @param broker Data structure that contains the connection information with the broker.
 * @param recv message_id.
 * @retval  1 On success.
 * @retval  0 On connection error.
 * @retval -1 On IO error.
 */
