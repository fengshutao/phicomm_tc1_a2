...
{
	u_printf("app_init\n");
	//启动时引脚默认输入开上拉，外部需挂下拉电阻，此处可以初始化为输出低低电平。
...
int USER_FUNC app_main (void)
{
	...
	//PWM初始化，频率4.9KHz，每个时间片100ns，2040x100ns=204us=4.9KHz.
	//GPIO3时间片必须是800ns的倍数,故满足0-255的范围的话，设置总tick 为255 X 8 =2040
	//设置占空比，只有设置完成之后才会引脚才会输出对应功能。
	//为了满足GPIO3的800ns时间片最小单位限制，设置其他PWM引脚统一为800ns,实际其他引脚时间片可支持0-2040级。
	//必须先初始化周期之后再设置占空比。
