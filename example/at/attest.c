...
#if (EXAMPLE_USE_DEMO==USER_AT_DEMO)

#define HFGPIO_F_ADC_CHANNEL1 HFGPIO_F_USER_DEFINE
#define HFGPIO_F_PWM_CHANNEL1 (HFGPIO_F_USER_DEFINE+1)
#define HFGPIO_F_PWM_CHANNEL2 (HFGPIO_F_USER_DEFINE+2)
#define HFGPIO_F_PWM_CHANNEL3 (HFGPIO_F_USER_DEFINE+3)
#define HFGPIO_F_PWM_CHANNEL4 (HFGPIO_F_USER_DEFINE+4)
...
const int hf_gpio_fid_to_pid_map_table[HFM_MAX_FUNC_CODE]=
{
	...
	LPx30_GPADC0,	   	//HFGPIO_F_USER_DEFINE
	LPx30_GPIO22,
	LPx30_GPIO23,
	LPx30_GPIO24,
	LPx30_GPIO25,
...
static int hf_atcmd_test(pat_session_t s,int argc,char *argv[],char *rsp,int len)
{
	...
	if(argc == 0)
	{
		...
		hffile_userbin_read(0, (char *)flash_data, 32);
		sprintf(rsp, "=%s", flash_data);
		return 0;
	}
	else if(argc == 1)
	{
		hffile_userbin_write(0, (char *)argv[0], 32);
	}
...
static int hf_atcmd_adctest(pat_session_t s,int argc,char *argv[],char *rsp,int len)
{
	...
	{
		case 0:
			hfgpio_adc_enable(HFGPIO_F_ADC_CHANNEL1);
			return 0;
		case 1:
			value = hfgpio_adc_get_value(HFGPIO_F_ADC_CHANNEL1);
			break;
		default:
			return -1;
	}
...
static int hf_atcmd_pwmtest(pat_session_t s,int argc,char *argv[],char *rsp,int len)
{
	int code, pin, fre, duty, fid;
	if(argc != 4)
		return -3;
	...
	{
		case 1:
			fid=HFGPIO_F_PWM_CHANNEL1;
			break;
		case 2:
			fid=HFGPIO_F_PWM_CHANNEL2;
			break;
		case 3:
			fid=HFGPIO_F_PWM_CHANNEL3;
			break;
		case 4:
			fid=HFGPIO_F_PWM_CHANNEL4;
			break;
		default:
			return -4;
	}
	if(fre<1 || fre>1000000)
		return -4;
	if(duty<0 || duty>100)
		return -4;
	
	if(code == 0)
		hfgpio_pwm_disable(fid);
	else if(code == 1)
		hfgpio_pwm_enable(fid, fre, duty);
...
const hfat_cmd_t user_define_at_cmds_table[]=
{
	...
	{"ADCTEST",hf_atcmd_adctest,"   AT+ADCTEST=code\r\n",NULL},	
	{"PWMTEST",hf_atcmd_pwmtest,"   AT+PWMTEST=code,pin,fre,duty\r\n",NULL},
