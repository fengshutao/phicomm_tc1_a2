...
#if (EXAMPLE_USE_DEMO==USER_JSON_DEMO)
#include "cJSON.h"
...
static int USER_FUNC socketa_recv_callback(uint32_t event,char *data,uint32_t len,uint32_t buf_len)
{
	if(event==HFNET_SOCKETA_DATA_READY)
	{
		...
		if(json == NULL)
		{
			u_printf("recv data is not a JSON!\r\n");
			return len;
		}
		...
		cJSON *pl = cJSON_GetObjectItem(json, "PL");
		if(pl != NULL)
		{
			cJSON *msg = cJSON_GetObjectItem(pl, "msg");
			if(msg != NULL)
				u_printf("JSON: msg = %s\r\n", msg->valuestring);
		}
		cJSON_Delete(json);
