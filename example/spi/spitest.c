...
#if (EXAMPLE_USE_DEMO==USER_SPI_DEMO)
...
int USER_FUNC app_main (void)
{
	...
	at25_init_spi();
	ret = at25_read_id(&p_id);
	u_printf("at25_read_id ret=%d, id=0x%x\r\n", ret, p_id);
	
	ret = at25_read(0, p_buf, sizeof(p_buf));
	u_printf("at25_read ret=%d, buf=[0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x]\r\n",
		ret, p_buf[0], p_buf[1], p_buf[2], p_buf[3], p_buf[4], p_buf[5], p_buf[6], p_buf[7]);
	
	ret = at25_erase_block_4k(0);
	u_printf("at25_erase_block_4k ret=%d\r\n", ret);

	for(int i=0; i<sizeof(p_buf); i++)
		p_buf[i]++;
	ret = at25_write(0, p_buf, sizeof(p_buf));
	u_printf("at25_write ret=%d\r\n", ret);
