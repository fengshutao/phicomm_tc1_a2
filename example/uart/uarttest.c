...
#if (EXAMPLE_USE_DEMO==USER_UART_DEMO)
...
void uart_test(void* arg)
{
	...
	hfuart_config(huart1, 115200, COMPARITY_NONE, COMBITS_8, COMSTOPBITS_1, COMUARTNFC);
    if(huart1==NULL)
    {
    ...
    while(1)
    {
        if(arg==(void*)1)
        {
            if(hfgpio_fpin_is_high(HFGPIO_F_NRELOAD)==0)
            {
                u_printf("switch to ap mode\n");
