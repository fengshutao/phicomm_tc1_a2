...
#if (EXAMPLE_USE_DEMO==USER_ALI_IOT_DEMO)
...
static int hf_atcmd_device(pat_session_t s,int argc,char *argv[],char *rsp,int len)
{
	if(argc == 0)
	{
        ...
        if((AliMqttConnectPara.head!=ALI_MQTT_CONNECT_PARA_HEAD)||(AliMqttConnectPara.tail!=ALI_MQTT_CONNECT_PARA_TAIL))
            sprintf(rsp, "=NULL,NULL");
        else
		    sprintf(rsp, "=%s,%s", AliMqttConnectPara.DeviceName,AliMqttConnectPara.DeviceSecret);
		return 0;
	}
	else if(argc == 2)
	{
		if(strlen(argv[0])>=sizeof(AliMqttConnectPara.DeviceName))
            return -4;
        else if(strlen(argv[1])>=sizeof(AliMqttConnectPara.DeviceSecret))
            return -4;
        ...
        memset(AliMqttConnectPara.DeviceName,0,sizeof(AliMqttConnectPara.DeviceName));
        memset(AliMqttConnectPara.DeviceSecret,0,sizeof(AliMqttConnectPara.DeviceSecret));
        strcpy(AliMqttConnectPara.DeviceName,argv[0]);
        strcpy(AliMqttConnectPara.DeviceSecret,argv[1]);
...
static int hf_atcmd_product(pat_session_t s,int argc,char *argv[],char *rsp,int len)
{
	if(argc == 0)
	{
        ...
        if((AliMqttConnectPara.head!=ALI_MQTT_CONNECT_PARA_HEAD)||(AliMqttConnectPara.tail!=ALI_MQTT_CONNECT_PARA_TAIL))
		    ...
		    sprintf(rsp, "=%s", AliMqttConnectPara.ProductKey);
		return 0;
	}
	else if(argc == 1)
	{
		if(strlen(argv[0])>=sizeof(AliMqttConnectPara.ProductKey))
            return -4;

        hfuflash_erase_page(ALI_MQTT_CONNECT_PARA_UFLASH_ADDR,1);
        AliMqttConnectPara.head=ALI_MQTT_CONNECT_PARA_HEAD;
        AliMqttConnectPara.tail=ALI_MQTT_CONNECT_PARA_TAIL;
        memset(AliMqttConnectPara.ProductKey,0,sizeof(AliMqttConnectPara.ProductKey));
        strcpy(AliMqttConnectPara.ProductKey,argv[0]);
        hfuflash_write(ALI_MQTT_CONNECT_PARA_UFLASH_ADDR,(char *)(&AliMqttConnectPara),sizeof(ALI_MQTT_CONNECT_PARA));
	}
...
static int hf_atcmd_topic(pat_session_t s,int argc,char *argv[],char *rsp,int len)
{
	if(argc == 0)
	{
        hfuflash_read(ALI_MQTT_CONNECT_PARA_UFLASH_ADDR,(char *)(&AliMqttConnectPara),sizeof(ALI_MQTT_CONNECT_PARA));
        if((AliMqttConnectPara.head!=ALI_MQTT_CONNECT_PARA_HEAD)||(AliMqttConnectPara.tail!=ALI_MQTT_CONNECT_PARA_TAIL))
		    ...
		    sprintf(rsp, "=%s,%s", AliMqttConnectPara.device_pubtopic,AliMqttConnectPara.device_subtopic);
		return 0;
	}
	else if(argc == 2)
	{
		if(strlen(argv[0])>=sizeof(AliMqttConnectPara.device_pubtopic))
            return -4;
        if(strlen(argv[0])>=sizeof(AliMqttConnectPara.device_subtopic))
            return -4;
        
        hfuflash_erase_page(ALI_MQTT_CONNECT_PARA_UFLASH_ADDR,1);
        AliMqttConnectPara.head=ALI_MQTT_CONNECT_PARA_HEAD;
        AliMqttConnectPara.tail=ALI_MQTT_CONNECT_PARA_TAIL;
        memset(AliMqttConnectPara.device_pubtopic,0,sizeof(AliMqttConnectPara.device_pubtopic));
        memset(AliMqttConnectPara.device_subtopic,0,sizeof(AliMqttConnectPara.device_subtopic));
        
        strcpy(AliMqttConnectPara.device_pubtopic,argv[0]);
        strcpy(AliMqttConnectPara.device_subtopic,argv[1]);
        hfuflash_write(ALI_MQTT_CONNECT_PARA_UFLASH_ADDR,(char *)(&AliMqttConnectPara),sizeof(ALI_MQTT_CONNECT_PARA));
	}
...
const hfat_cmd_t user_define_at_cmds_table[]=
{
    ...
    {"PRODUCT",hf_atcmd_product,"   AT+PRODUCT: Set/Get ProductKey.\r\n",NULL},
    {"TOPIC",hf_atcmd_topic,"   AT+TOPIC: Set/Get device_pubtopic/device_subtopic.\r\n",NULL},
...
static int USER_FUNC uart_recv_callback(uint32_t event,char *data,uint32_t len,uint32_t buf_len)
{
    ...
    ali_mqtt_message_publish(mqtt_pub_topic,data,len,QOS1);
	return 0;
}
