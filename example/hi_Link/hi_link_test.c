...
#if (EXAMPLE_USE_DEMO==USER_HI_LINK_DEMO)

#include "../../thirdpartylib/hilinksdk/include/hilink.h"
#include "../../thirdpartylib/hilinksdk/include/hilink_osadapter.h"


#define EXAMPLE_VER " HiLink_example "
...
static int hf_cmd_get_version(pat_session_t s,int argc,char *argv[],char *rsp,int len)
{
	if(argc==0)
	{ 
		sprintf(rsp,"=%s(%s %s)",EXAMPLE_VER,__DATE__,__TIME__);
		return 0;
	}
	return -3;
}

static int hf_cmd_enter_hilink(pat_session_t s,int argc,char *argv[],char *rsp,int len)
{
	if(argc==0)
	{
    	char rsp[64]={0};
    	hfat_send_cmd("AT+WMODE=AP\r\n",sizeof("AT+WMODE=AP\r\n"),rsp,64);
        hfuart_send(HFUART0,"+ok",3,0);
...
const hfat_cmd_t user_define_at_cmds_table[]=
{
	...
	{"TESTHILINK",hf_cmd_enter_hilink,"   AT+TESTHILINK: test hilink linking\r\n", NULL},
...
void USER_FUNC hilink_example_start(void)
{
   
    u_printf(" --- [example] ---\r\n");
    hilink_printf("hilink example [init] \r\n");

    //start everything of the HiLink
    hilink_main();
    
    hilink_printf("hilink example [start]\r\n");
    u_printf("happy ending\r\n");
}
...
int USER_FUNC app_main (void)
{
    ...
    //HiLink : start everything of the HiLink
    hilink_example_start();
